...
contract TestDeployAndExactSwap is Script {
...
*/
        ...
        CamelotHook hook = CamelotHook(address(0x9B21E1cdD975E58F714Fd1Bd5a803198d5520088));
        ...
        PoolKey memory key2 = PoolKey({
            currency0: Currency.wrap(token0),
            currency1: Currency.wrap(token1),
            fee: 500, // TODO: Replace with actual fee tier
            tickSpacing: 10, // TODO: Replace with actual tick spacing
        ...
        uint128 amountIn = 0.004 * 10**18; // 0.01 WETH
...
///////////////////////////
        ...
        // Measure balance before swap
        balanceBefore = IERC20(token1).balanceOf(address(msg.sender));

        // Execute the swap
        UniversalRouter(payable(router)).execute(commands, inputs, block.timestamp * 2);

        // Measure balance after swap
        balanceAfter = IERC20(token1).balanceOf(address(msg.sender));
